...
public class RRAutoObservation_v3 extends LinearOpMode {
    ...
    Trajectory preparePush, initialMovement, pushSamples1, pushSamples2, pushSamples3;
    TrajectorySequence parking;

    double maxVel, maxAccel, pushMaxAccel, pushMaxVel;
            ...
            switch (movestep) {
                ...
                case SCORING_SPECIMEN_1:
                    ...
                    if (robot.arm.getCurrentPosition() >= 678) {
                        ...
                        if (robot.arm.getCurrentPosition() >= 702) {
                ...
                case PREPARE_PUSH:
                    ...
                    pushSamples1 = drive.trajectoryBuilder(currentPos)
                            ...
                            .splineToConstantHeading(new Vector2d(37.00, -10.00), Math.toRadians(40.00)
                                    ,SampleMecanumDrive.getVelocityConstraint(maxVel, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                                    SampleMecanumDrive.getAccelerationConstraint(maxAccel)
                            )
                            .splineToConstantHeading(new Vector2d(47.00, -56.00), Math.toRadians(-85.00)
                                    ,SampleMecanumDrive.getVelocityConstraint(maxVel, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                                    SampleMecanumDrive.getAccelerationConstraint(maxAccel)
                            )
                            .addTemporalMarker(1.00, 0.00, () -> {
